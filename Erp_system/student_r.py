...
class Database:
    def __init__(self, master, *args, **kwargs):
        ...
        self.heading = tkinter.Label(
            master, text="Student Registration",  fg='steelblue', font=('arial 40 bold'))
        ...
        self.name_l = tkinter.Label(
            master, text="Enter Student's Name", font=('arial 18 bold'))
        self.name_l.place(x=0, y=200)

        self.course_l = tkinter.Label(
            master, text="Enter Student's Course", font=('arial 18 bold'))
        self.course_l.place(x=0, y=300)

        self.branch_l = tkinter.Label(
            master, text="Enter Student's Branch", font=('arial 18 bold'))
        self.branch_l.place(x=0, y=400)

        self.attend_l = tkinter.Label(
            master, text="Enter Student's Attendance", font=('arial 18 bold'))
        ...
        self.btn_add = tkinter.Button(
            master, text="ADD", width=20, height=2, bg='steelblue', command=self.write_stu)
        self.btn_add.place(x=585, y=550)

        # clear field button
        self.btn_clr = tkinter.Button(
            master, text="CLEAR", width=20, height=2, bg='steelblue', command=self.clear_all)
    ...
    def write_stu(self, *args, **kwargs):
        ...
        if self.name == '' or self.roll == '' or self.course == '' or self.branch == '' or self.attend == '':
            tkinter.messagebox.showwarning(
                "Warning", "PLEASE FILL ALL THE ENTRIES")
        else:

            sql = "INSERT INTO student ( id , name , course , branch , attendance ) VALUES(?,?,?,?,?)"
            c.execute(sql, (self.roll, self.name,
                            self.course, self.branch, self.attend))
            conn.commit()
            # text box msg
            self.tBox.insert(tkinter.END, "\n\n Inserted " + str(self.name) +
